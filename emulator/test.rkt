...
(define-test-suite utils
  ...
  (test-case "pmp-napot-settings"
             ...
             (check-equal? (pmpcfg_setting-R setting1) (bv 1 1))
             (check-equal? (pmpcfg_setting-W setting1) (bv 1 1))
             (check-equal? (pmpcfg_setting-A setting1) (bv 3 2))
             (check-equal? (pmpcfg_setting-X setting1) (bv 1 1))
             (check-equal? (pmpcfg_setting-L setting1) (bv 0 1))
             (define setting2 (pmp-decode-cfg (bv #x0000000000001f1f 64) 2))
             (check-equal? (pmpcfg_setting-R setting2) (bv 0 1))
             (check-equal? (pmpcfg_setting-W setting2) (bv 0 1))
             (check-equal? (pmpcfg_setting-A setting2) (bv 0 2))
             (check-equal? (pmpcfg_setting-X setting2) (bv 0 1))
             (check-equal? (pmpcfg_setting-L setting2) (bv 0 1))
             (define setting5 (pmp-decode-cfg (bv #x0000000000001f1f 64) 5))
             (check-equal? (pmpcfg_setting-R setting5) (bv 0 1))
             (check-equal? (pmpcfg_setting-W setting5) (bv 0 1))
             (check-equal? (pmpcfg_setting-A setting5) (bv 0 2))
             (check-equal? (pmpcfg_setting-X setting5) (bv 0 1))
             (check-equal? (pmpcfg_setting-L setting5) (bv 0 1)))
             ...
             (define model_mode
               (verify (assert
                        (or (equal? (machine-mode m) (machine-mode m1))
                            (and (bveq (get-pc m)
                                       (bvsub (get-csr m 'mtvec) (base-address)))
